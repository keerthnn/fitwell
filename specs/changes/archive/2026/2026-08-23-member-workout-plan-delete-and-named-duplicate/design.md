...
>
> **Depends on:** Originally approved [Clarify](clarify.md) and [Proposal](proposal.md). Approval and verification history are retained.
...
| --- | --- |
| Code | Plan detail page, `src/components/workout-plans/`, plan API handlers/validator, and shared client; complete paths in frontmatter |
| Data | `WorkoutPlan`, `WorkoutPlanExercise`, and nullable `Workout.sourceWorkoutPlanId`; existing schema and referential actions |
| Tenants / tiers | Authenticated members; only owners delete private plans; visible private/built-in sources may be duplicated |
...
| Concern | Link or N/A |
| --- | --- |
| Vendors | N/A — no external service change |
| Deployment / flags | No new flags or configuration; release and recovery constraints below |
