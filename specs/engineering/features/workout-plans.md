...
requirements: [PLAN-001, PLAN-002, PLAN-003, PLAN-004, PLAN-005, PLAN-006, PLAN-007, PLAN-008, PLAN-009, PLAN-010, PLAN-011, PLAN-012, PLAN-013, PLAN-014, SEC-002, SEC-004, DATA-002, DATA-006, A11Y-002, A11Y-004, A11Y-005]
...
- `/workout-plans/create` uses muscle-guided discovery to build a private plan while keeping prescriptions in the owning form.
...
`WorkoutPlanForm` manages basics and ordered prescriptions. Member creation composes muscle-guided discovery; member edit and administrator create/edit retain the legacy search picker. Filter, view, request, and page changes do not reconstruct the form's selected exercise or prescription maps. `WorkoutPlanExercisePrescription` edits sets/rep range/rest. `WorkoutPlanList/Card` renders the library. `WorkoutPlanDetail` and `WorkoutPlanVisual` present details and cover resolution. `DuplicateWorkoutPlanButton` owns the copy-name dialog and request state; `DeleteWorkoutPlanButton` owns the irreversible confirmation and delete request state.

## API and database usage

Create validates 1–100 prescriptions and confirms all referenced exercises are active immediately before its one nested aggregate create. Update first verifies owned/private, then transactionally deletes all prescriptions and recreates them with updated plan data. Archive toggles `isArchived` on owned private plans. Duplicate validates a caller-submitted required name of at most 120 characters, loads any visible source, and copies it into a private non-built-in aggregate owned by the caller.

Muscle selection and browsing do not create a plan or start a workout. Only explicit plan submission persists the selected ordered exercises and prescriptions; an exercise archived after discovery is rejected without creating a partial plan.
...
- Focused create active-reference, muscle-guided form, duplicate/delete validation, ownership predicates, UI confirmation, and schema lifecycle contracts have automated coverage. Visibility, update prescription, start, seed execution, and representative database referential-action behavior still lack integration evidence.
