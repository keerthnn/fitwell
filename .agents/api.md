...
5. Validate request data with the pure runtime validators in `src/lib/api/validators/`. Return structured field-level `400` responses.
...
Workout Plans are either explicitly built-in (`userId: null`, `isBuiltIn: true`) or private (`userId: authenticatedUserId`, `isBuiltIn: false`). A null owner is never sufficient authorization.
