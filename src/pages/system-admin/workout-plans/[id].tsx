...
import {
  adminArchiveWorkoutPlan,
  adminRestoreWorkoutPlan,
  getWorkoutPlan,
...
export default function AdminWorkoutPlanPage() {
  ...
  useEffect(() => {
    if (id) void getWorkoutPlan(id, true).then(setPlan);
  }, [id]);
  return (
    <AdminLayout>
      {!plan ? (
        ...
        <>
          ...
          <Paper variant="outlined" sx={{ p: 3 }}>
            <Stack gap={3}>
              <WorkoutPlanForm initial={plan} admin />
              <Button
                color={plan.isArchived ? "success" : "warning"}
                variant="outlined"
                onClick={async () => {
                  if (plan.isArchived) await adminRestoreWorkoutPlan(plan.id);
                  else await adminArchiveWorkoutPlan(plan.id);
                  await router.push("/system-admin/workout-plans");
                }}
              >
                {plan.isArchived
