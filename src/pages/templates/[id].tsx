...
import { copyTemplate, createWorkout, getTemplate } from "fitness/utils/spec";
...
export default function TemplateDetailPage() {
  ...
  useEffect(() => {
    if (id) getTemplate(id).then(setItem);
  }, [id]);
  if (!item)
    return (
      <AuthenticatedPage>
      ...
      <Container maxWidth="md" sx={{ py: 4 }}>
        <Stack spacing={3}>
          <Paper sx={{ p: 4 }}>
            ...
            {item.description && (
              <Typography mt={3}>{item.description}</Typography>
            )}
            <Typography mt={2}>
              {item.estimatedDurationM ?? "—"} minutes · {item.difficulty} ·{" "}
              {item.useCount} uses
            </Typography>
          </Paper>
          ...
          {item.exercises.map((target) => (
            <Paper key={target.exerciseId} sx={{ p: 3 }}>
              <Typography fontWeight={600}>{target.exercise?.name}</Typography>
              <Typography color="text.secondary">
                {target.targetSets} sets × {target.minReps ?? "—"}
                {target.maxReps ? `–${target.maxReps}` : ""} reps · rest{" "}
                {target.restSeconds ?? "—"} sec
              </Typography>
            </Paper>
          ))}
        </Stack>
