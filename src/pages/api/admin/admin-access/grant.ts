...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const target = await prisma.user.findFirst({ where: { id: result.data.id, isDisabled: false, deletedAt: null } });
  if (!target) return res.status(404).json({ error: "Active user not found" });
  await prisma.$transaction(async (tx) => {
    await tx.adminAccess.upsert({ where: { userId: target.id }, create: { userId: target.id, grantedById: adminId }, update: {} });
    await tx.adminAuditLog.create({ data: auditData(adminId, "ADMIN_GRANTED", "AdminAccess", target.id) });
  });
