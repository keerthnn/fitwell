...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!isIdentifier(id)) return res.status(400).json({ error: "Invalid exercise id" });
  const result = validateExercise(req.body);
  if (!result.valid) return res.status(400).json({ errors: result.errors });
  const exercise = await prisma.$transaction(async (tx) => {
    const updated = await tx.exercise.update({ where: { id }, data: result.data });
    await tx.adminAuditLog.create({ data: auditData(adminId, "EXERCISE_UPDATED", "Exercise", id) });
    return updated;
  });
