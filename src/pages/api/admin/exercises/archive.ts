...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  await prisma.$transaction(async (tx) => {
    await tx.exercise.update({ where: { id: result.data.id }, data: { isActive: false } });
    await tx.adminAuditLog.create({ data: auditData(adminId, "EXERCISE_ARCHIVED", "Exercise", result.data.id) });
  });
