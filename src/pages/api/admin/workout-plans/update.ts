...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!isIdentifier(id)) return res.status(400).json({ error: "Invalid Workout Plan id" });
  const result = validateWorkoutPlan(req.body);
  if (!result.valid) return res.status(400).json({ errors: result.errors });
  const existing = await prisma.workoutPlan.findFirst({ where: { id, userId: null, isBuiltIn: true } });
  if (!existing) return res.status(404).json({ error: "Built-in Workout Plan not found" });
  const { exercises, ...data } = result.data;
  const plan = await prisma.$transaction(async (tx) => {
    await tx.workoutPlanExercise.deleteMany({ where: { workoutPlanId: id } });
    const updated = await tx.workoutPlan.update({ where: { id }, data: { ...data, exercises: { create: exercises.map((item) => ({
    ...
    await tx.adminAuditLog.create({ data: auditData(adminId, "WORKOUT_PLAN_UPDATED", "WorkoutPlan", id) });
    return updated;
  });
