...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const changed = await prisma.$transaction(async (tx) => {
    const update = await tx.workoutPlan.updateMany({ where: { id: result.data.id, userId: null, isBuiltIn: true }, data: { isArchived: false, isActive: true } });
    if (update.count) await tx.adminAuditLog.create({ data: auditData(adminId, "WORKOUT_PLAN_RESTORED", "WorkoutPlan", result.data.id) });
    return update.count;
  });
