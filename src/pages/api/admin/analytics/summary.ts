...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const [completedWorkouts, activeUsers, duration] = await Promise.all([
    ...
    prisma.workout.aggregate({ where: { status: "COMPLETED" }, _sum: { durationMinutes: true } }),
  ]);
  return res.status(200).json({ completedWorkouts, activeUsers, durationMinutes: duration._sum.durationMinutes ?? 0 });
}
