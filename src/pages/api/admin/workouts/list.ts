...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const items = await prisma.workout.findMany({ include: { user: { select: { email: true, displayName: true } }, _count: { select: { exercises: true } } }, orderBy: { workoutDate: "desc" }, take: 100 });
