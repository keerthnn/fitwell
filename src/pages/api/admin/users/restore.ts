...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!target || target.deletedAt) return res.status(409).json({ error: "Deleted accounts cannot be restored by this operation" });
  await prisma.$transaction(async (tx) => {
    await tx.user.update({ where: { id: result.data.id }, data: { isDisabled: false } });
    await tx.adminAuditLog.create({ data: auditData(adminId, "USER_RESTORED", "User", result.data.id) });
  });
