...
export function validateExercise(value: RequestInputValue) {
  ...
  const primaryMuscle = text(input.primaryMuscle, "primaryMuscle", errors, {
    required: true,
    max: 80,
  });
  ...
  const instructions = text(input.instructions, "instructions", errors, {
    max: 5000,
  });
  const imagePath = text(input.imagePath, "imagePath", errors, { max: 300 });
  const thumbnailPath = text(input.thumbnailPath, "thumbnailPath", errors, {
    max: 300,
  });
  const equipmentImagePath = text(
    input.equipmentImagePath,
    "equipmentImagePath",
    errors,
    { max: 300 },
  );
